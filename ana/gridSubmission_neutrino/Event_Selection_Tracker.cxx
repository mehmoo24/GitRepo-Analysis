...
//  std::cout << option << std::endl;
  ...
  const string playlist= argv[2];
    ...
    const string name_of_file = argv[3]; // name of file (ideally include the date)

    //const string fewer_files_opt = argv[4];
    const string fewer_files_opt = "";

    const std::string mc_file_list(Form("../include/playlists/MAD_tuples_p4_Neutrino/mad_mc_%s%s.txt", plist_string.c_str(), fewer_files_opt.c_str()));
    const std::string data_file_list(Form("../include/playlists/MAD_tuples_p4_Neutrino/mad_data_%s%s.txt",plist_string.c_str(), fewer_files_opt.c_str()));
...
//  PlotUtils::MacroUtil util(reco_tree_name, mc_file_list, data_file_list, plist_string, wants_truth, is_grid);
  ...
  PlotUtils::MinervaUniverse::SetNuEConstraint(true); //true
  ...
  for (auto v : variables2DMC) {v->m_selected_mc_reco.SyncCVHistos();
                                ...
                                v->signal_purityNum.SyncCVHistos();
                                v->bkg_total.SyncCVHistos();
                                v->bkg_QE.SyncCVHistos();
                                v->bkg_2p2h.SyncCVHistos();
                                v->bkg_trueDIS.SyncCVHistos();
                                v->bkg_softDIS.SyncCVHistos();
                                v->bkg_other.SyncCVHistos();
                                v->bkg_resonant.SyncCVHistos();
                                v->ANCC.SyncCVHistos(); v->ANNC.SyncCVHistos(); v->NCC.SyncCVHistos(); v->NNC.SyncCVHistos(); v->ANNCCNCother.SyncCVHistos();
  }
...
//  auto data5Cut = new TParameter<int>("DataMinus9999_ifapplied", data5);
...
//  auto mcreco5Cut = new TParameter<int>("MCRecoMinus9999_ifapplied", mcreco5);
        ...
        if(isMC){     
          for (auto band : error_bands){
            ...
            for (auto universe : error_band_universes){
              ...
              // look at the -9999 events
              //if ((universe->GetInt("MasterAnaDev_minos_trk_end_x") != -9999)) continue;
              //if ((universe->GetInt("MasterAnaDev_minos_trk_end_y") != -9999)) continue;
              reco5++;
              mcreco5++;
              ...
              for (auto v : variables2d){
	            ...
	            v->m_selected_mc_reco.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
                    ...
                    // now the events that pass the signal definition
                    //if ((universe->GetInt("mc_incoming") == 14) && (universe->GetInt("mc_current") == 1)&&(cutter->PassTrueThetaCut(universe))){   
                     if ((universe->GetInt("mc_incoming") == 14) && (universe->GetInt("mc_current") == 1)){
                           v->signal_purityNum.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
                           mcsignal++;

 // COMMNETED OUT BREAKDOWN                         
                           // breakdown of signal events by interaction channel
...
*/
                            // breakdown - method #2
/*
                            if (universe->GetInt("mc_intType") == 1) v->bkg_QE.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());      
                            else if (universe->GetInt("mc_intType") == 8) v->bkg_2p2h.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());  
                            else if (universe->GetInt("mc_intType") == 3){
                                  if (cutter->PassDISCut(universe)) v->bkg_trueDIS.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
                                   else {v->bkg_softDIS.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());}
                            }     
                            else if (universe->GetInt("mc_intType") == 2) v->bkg_resonant.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());
                            else {v->bkg_other.univHist(universe)->Fill(v->GetRecoValueX(*universe), v->GetRecoValueY(*universe), universe->GetWeight());}
*/
              ...
              if ((universe->GetInt("mc_incoming") == 14) && (universe->GetInt("mc_current") == 1)){
                           v->signal_purityNum.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                           // breakdown of signal events by interaction channel
                    if (universe->GetInt("mc_intType") == 1) v->reco_QE.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                    else if (universe->GetInt("mc_intType") == 8) v->reco_2p2h.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                    else if (universe->GetInt("mc_intType") == 3){
                            if (cutter->PassDISCut(universe)) v->reco_trueDIS.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            else {v->reco_softDIS.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());}
                    }
                   else if (universe->GetInt("mc_intType") == 2) v->reco_resonant.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                    else {v->reco_other.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());}
                   } // end signal if statement
              else{
                            v->bkg_total.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                  if ((universe->GetInt("mc_incoming") == -14) && (universe->GetInt("mc_current") == 1)) v->ANCC.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            else if ((universe->GetInt("mc_incoming") == -14) && (universe->GetInt("mc_current") != 1)) v->ANNC.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            else if ((universe->GetInt("mc_incoming") == 14) && (universe->GetInt("mc_current") == 1)) v->NCC.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            else if ((universe->GetInt("mc_incoming") == 14) && (universe->GetInt("mc_current") != 1)) v->NNC.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());
                            else {v->ANNCCNCother.univHist(universe)->Fill(v->GetRecoValue(*universe, 0), universe->GetWeight());}
              }
              }              
	 ...
	 reco1++;
         ...
         // -9999 events
         //if ((dataverse->GetInt("MasterAnaDev_minos_trk_end_x") != -9999)) continue;
         //if ((dataverse->GetInt("MasterAnaDev_minos_trk_end_y") != -9999)) continue;
              reco5++;
              data5++;
