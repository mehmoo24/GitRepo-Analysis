...
//  float  pot_x = 0; // commented out May18/22
  ...
  while((ob=next())){
    ...
    // added for the counters (Jan 9,2024)
    else if(classname == "TParameter<int>"){
      int counterOut = 0;
      for(int i=0;i<num_playlists;i++){
        TParameter<int> *constant = (TParameter<int>*)f_input[i]->Get(name.c_str());
        counterOut = counterOut + constant->GetVal(); 
     } 
      auto counterOutYes = new TParameter<int>(name.c_str(), counterOut);
      f_output->cd();
      counterOutYes->Write();
      delete counterOutYes;   
    } // end class name TParameter<int>
