...
//void SetupResponse(T univs){
	   ...
	   cout << "The response is being setup now" << endl;
	   const char* name = GetName().c_str();
	   ...
	   //bin_x.uniform=false;
           //bin_y.uniform=false;
     	   ...
     	   vector<double> vx;
	   for(int i=0; i<=GetNBinsX(); i++){vx.push_back(GetBinVecX().data()[i]); cout << GetBinVecX().data()[i] << endl;}

           //vx[GetNBinsX()+1] = vx[GetNBinsX()]+5;
	   for(int i=0; i<=GetNBinsX()+1; i++){cout << "i: " << i << " vx: " << vx[i] << endl;}
	   cout << "Check 1" << endl;

	   vector<double> vy;
           for(int j=0; j<=(GetNBinsY()); j++){vy.push_back(double(GetBinVecY().data()[j])); cout << GetBinVecY().data()[j] << endl;}
           for(int j=0; j<=GetNBinsY(); j++){cout << "j: " << j << " vy: " << vy[j] << endl;}
	   cout << "Check 2" << endl;

	   PlotUtils::AnaBinning anabin;
		   
           anabin.AddBins(vx,5.0, 60.0, 65.0);
	   anabin.AddBins(vx,5.0, 65.0, 70.0);
           anabin.AddBins(vy,1.0,1.5,2.5);
	   anabin.AddBins(vy,2.0,2.5,4.5);
	   cout << "New vx now " << endl;
	   for(int i=0; i<=GetNBinsX(); i++){cout << "i: " << i << " vx: " << vx[i] << endl;}
	   bin_x.bin_edges = vx;
	   bin_x.nbins	    = GetNBinsX();
	   cout << "bin_x.nbins : " <<  GetNBinsX() << endl;
           //bin_x.nbins = bin_x.nbins+1; // line added b/c otherwise response wasn't filling up
	   cout << "NEW bin_x.nbins : " <<  bin_x.nbins << endl;
	   bin_x.min 	    = GetBinVecX().data()[0];
           cout << "bin_x.min: " << bin_x.min << endl;
	  // bin_x.max = GetBinVecX().data()[bin_x.nbins];
	   bin_x.max       = GetBinVecX().data()[GetNBinsX()];
	   cout << "bin_x.max: " << bin_x.max << endl;
	   bin_y.bin_edges = vy;
	   bin_y.nbins	    = GetNBinsY();
	   cout << "bin_y.nbins : " <<  GetNBinsY() << endl;
	   bin_y.nbins = bin_y.nbins; // line added b/c otherwise response wasn't filling up
           cout << "NEW bin_y.nbins : " <<  bin_y.nbins << endl;
	   bin_y.min 	    = GetBinVecY().data()[0];

	   //bin_y.max       = GetBinVecY().data()[bin_y.nbins];
	   bin_y.max       = GetBinVecY().data()[GetNBinsY()];
	   bin_y.max = bin_y.max; 
           //bin_y.max = 2.5;

           for(int j=0; j<=bin_y.nbins; j++){cout << "j: " << j << " bin_y: " << bin_y.bin_edges[j] << endl;}


	   cout << "bin_y.min: " << bin_y.min << endl;
           cout << "bin_y.max: " << bin_y.max << endl;
	   ...
	   cout << "The response has finished setting up now" << endl;
}
