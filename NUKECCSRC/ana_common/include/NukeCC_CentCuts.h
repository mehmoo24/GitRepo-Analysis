 ...
 class PassMuQualityCut:public Cut<UNIVERSE, EVENT> 
{
   ...
   //! If the minos track quality is unknown or not set, return false
    if( univ.GetDouble("MasterAnaDev_minos_trk_quality") <= 0 )
        return false;
    //! If the minos track quality is equal to or better than requested, return true
    return ( univ.GetDouble("MasterAnaDev_minos_trk_quality") <= m_qual );
    }
    int m_qual;
};
...
{  
  public:
    ...
    double maxZ = 6000.;
    if ( univ.GetInt("MasterAnaDev_targetID") == 0 && ( minZ <= univ.GetVecElem("MasterAnaDev_vtx",2) && univ.GetVecElem("MasterAnaDev_vtx",2) <= maxZ ) ){
       return true;
    }
    //passive target ZDist cut is made with nPlanes cut in framework
    if( 1 <= univ.GetInt("MasterAnaDev_targetID") && univ.GetInt("MasterAnaDev_targetID") <= 5 )
    {
    ...
    if( FIRST_TRACKER_MOD <= univ.GetInt("MasterAnaDev_vtx_module") && univ.GetInt("MasterAnaDev_vtx_module") <= LAST_TRACKER_MOD )
        return true;
  ...
  class PassDistToDivisionCut:public Cut<UNIVERSE, EVENT>
{
     ...
     bool checkCut(const UNIVERSE& univ,  EVENT& /*evt*/) const override  
  {
    ...
    if( 0 < univ.GetInt("MasterAnaDev_targetID") && univ.GetInt("MasterAnaDev_targetID") < 10 && 4 != univ.GetInt("MasterAnaDev_targetID") )
        return ( m_xySep < univ.GetDouble("MasterAnaDev_target_dist_to_division" ));
...
bool IsInMaterial_b(const UNIVERSE& univ,int m_targetID,  int m_targetZ, bool m_anyTrackerMod  = false ) const {
    if( m_targetID < 0)
    {
        // if targetID < 0, then we want any event in the nuclear target region mods -5-26
         double z = univ.GetVecElem("MasterAnaDev_vtx",2);
        double minZ = 4290.; // z center of mod -5 plane 1 is 4293.04mm, while plane 2 is 4313.68mm
        double maxZ = 6000.;
        if ( !( minZ <= z && z <= maxZ ) )
            return false;
        ...
        if( m_targetZ > 0 && ! IsInTargetSection(m_targetID, m_targetZ, univ.GetVecElem("MasterAnaDev_vtx",0),univ.GetVecElem("MasterAnaDev_vtx",1) ) )
          return false;
    ...
    if(m_targetID < 10 )
    {
      ...
      if(univ.GetInt("MasterAnaDev_targetID") == 0 && univ.GetDouble("MasterAnaDev_vtx_module") < 27 )
      {
	if( m_targetZ > 0 && IsInTargetSection( m_targetID, m_targetZ, univ.GetVecElem("MasterAnaDev_vtx",0),univ.GetVecElem("MasterAnaDev_vtx",1) ) )
	  return true;
      }
      ...
      if( univ.GetInt("MasterAnaDev_targetID") == m_targetID )
      {
	if( m_targetZ > 0 )
	  return univ.GetInt("MasterAnaDev_targetZ") == m_targetZ;
	else
	  return true;
    ...
    else
    {
        ...
        if( m_targetZ > 0 && univ.GetVecElem("MasterAnaDev_ref_targZ",(refTarg-1)) != m_targetZ )
            return false;
        ...
        if( firstMod <= univ.GetDouble("MasterAnaDev_vtx_module") && univ.GetDouble("MasterAnaDev_vtx_module") <= lastMod )
            return true;
    }
    return false;
...
int m_targetID;
   ...
   template <class UNIVERSE, class EVENT = detail::empty>
   class InHexagon: public Cut<UNIVERSE, EVENT>
   {
   public:
       InHexagon(double apothem = 850.):Cut<UNIVERSE, EVENT>("In Hexagon Cut"){}
   private:
     bool checkCut(const UNIVERSE& univ, EVENT& /*evt*/) const override
   {   
    ...
    double x = univ.GetVecElem("MasterAnaDev_vtx",0);
    double y = univ.GetVecElem("MasterAnaDev_vtx",1);
...
template <class UNIVERSE, class EVENT = detail::empty>
cuts_t<UNIVERSE, EVENT> getCCInclusiveCutsTracker( HelicityType::t_HelicityType h)
  {
   cuts_t<UNIVERSE, EVENT> inclusive_cuts;
   const double minZ = 5980, maxZ = 8422;
//   inclusive_cuts.emplace_back(new ZRange<UNIVERSE, EVENT>("Tracker", minZ, maxZ));
//   inclusive_cuts.emplace_back(new PassHelicityCut<UNIVERSE, EVENT>(h));
   inclusive_cuts.emplace_back(new InHexagon<UNIVERSE, EVENT>());
